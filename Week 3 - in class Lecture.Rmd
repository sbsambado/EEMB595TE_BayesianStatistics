---
title: "Week 3 - In class Lecture"
author: "sbsambado"
date: "4/15/2020"
output: html_document
---
zoom meeting class with Dr. Steph Prouxl
...
## in stan you have to tell it what its bands are, etc.

# what stan does and whatever mcmc algorithms do, for any given set parameters it caluclates posterior probability, which involves calculating likelihood
# how it choses values matter 
# ~ line calculates component of that posterior probability 
# it does the same for wach data point, adds them up and gets total weight of those parameters
...
print(stanfit_gauss)
pairs(stanfit_gauss,pars=c("mu", pars="sigma") ) # plots histogram 

# R hat value tells you how important the values of the chains. if chains do a good job of being mix, it will be close to 1. A number bigger than 1, something isn't right
```

We can use the tools to quantify a posterior sample. Once extracted, you can do anything you like with them!
R4.32-33
```{r posterior_samples}
# if you have multiple packages missing, or give rstan :: extract function 
post <- rstan ::extract(stanfit_gauss,pars=c("mu","sigma"))
...
## another program block to transform parameters
# the program has a data block, the data tells stan what the data commands and looksl ike
...
linear_model <- stan_model('model_wheight.stan') # compile stan file
...
pairs(stanfit_linear, pars=c("alpha","beta","sigma")) # pairs function will plot

# printing all transformed variables

## okay if there's some covariance in parameters if its a high level model

# this model doesnt cleayl tell me what alpha is vs beta is, doesnt allow individual parameters as well
```
...
If you have uninformative variables, the model can wander arround. aggregate result would be the same
R4.42
```{r center_data}
# recentering dta to find residual weight of mean
# library(tidyverse)
d2 <- 
  d2 %>%
...
mean_params <- get_posterior_mean(stanfit_linear , pars=c("alpha","beta")) # to get table

  ggplot(d2,aes(x = weight, y = height)) +
  geom_abline(intercept = mean_params[1,5], # row 1 column 5
              slope     = mean_params[2,5]) +
  geom_point(shape = 1, size = 2, color = "royalblue") +
  theme_bw() +
  theme(panel.grid = element_blank())
# what is the actual way of outcomes with this set of data
  # can be used for power analyses
```
...
## Non-linear fit (quadratic)

Here we just add one more term to our model for the quadratic component of the fit.
```{r write_stan_quadratic}
sink("model_quad.stan")
cat("
...
quad_model <- stan_model('model_quad.stan')


```


We'll do the centering again here before running the stan model. Here we use the full dataset including young people, which makes the dataset have more curvature. You may also want to try running this with just the individuals >18 years old and compare your results. Since a linear fit works well for the >18yo you can ask how you could tell this from our fit. 
...
d3 <- 
  d %>%
  mutate(weight_c = weight - mean(weight))
dcentered=select(d3,-weight) %>% rename(weight=weight_c)
stan_data <- c(dcentered[c("height","weight")], list(N = length(dcentered$height)))



stanfit_quad <- sampling(quad_model, data = stan_data, chains = 4,
                      iter = 4000, seed = 2131231)
```


Again we print the output nad plot the posterior distributions. There is some correlation among the parmaeters, even after centering.
```{r print_stan_quad, eval=FALSE}

print(stanfit_quad , pars=c("alpha","beta","gamma","sigma"))
pairs(stanfit_quad, pars=c("alpha","beta","gamma","sigma"))

```

```{r plot_quad_fit}

mean_params <- get_posterior_mean(stanfit_quad , pars=c("alpha","beta","gamma"))

d3<- mutate(d3,quad_fit = mean_params[1,5]+mean_params[2,5] * weight_c +mean_params[3,5]*weight_c^2 )

  ggplot(d3,aes(x = weight, y = height)) +
    geom_line(aes(x=weight, y=quad_fit))+
    geom_point(shape = 1, size = 2, color = "royalblue") +
  theme_bw() +
  theme(panel.grid = element_blank())
